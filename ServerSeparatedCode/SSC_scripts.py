...
# In the script "player_respawn_in_place", after the following line:
[(store_script_param, ":player_id", 1), # must be valid
# Add the following line:
(player_set_slot, ":player_id", slot_player_respawninsameplace, 1),
...
("game_receive_url_response", # called by the game when a response is received from a web server, if used
   [#(store_script_param, ":integer_count", 1),
    ...
    (try_begin),
      ...
      (else_try),
        (player_set_slot, ":player_id", slot_player_saved_item_0, 0),
      (try_end),
      (try_begin),
        (gt, reg9, 0),
        (player_set_slot, ":player_id", slot_player_saved_item_1, reg9),
      (else_try),
        (player_set_slot, ":player_id", slot_player_saved_item_1, 0),
      (try_end),
      (try_begin),
        (gt, reg10, 0),
        (player_set_slot, ":player_id", slot_player_saved_item_2, reg10),
      (else_try),
        (player_set_slot, ":player_id", slot_player_saved_item_2, 0),
      (try_end),
      (try_begin),
        (gt, reg11, 0),
        (player_set_slot, ":player_id", slot_player_saved_item_3, reg11),
      (else_try),
        (player_set_slot, ":player_id", slot_player_saved_item_3, 0),
      (try_end),
      (try_begin),
        (gt, reg12, 0),
    ...
    (else_try),
      ...
      (try_begin),
        ...
        (gt, reg4, 0),
        (init_position, pos62),
        (position_set_x, pos62, reg2),
        (position_set_y, pos62, reg3),
        (position_set_z, pos62, reg4),
        (agent_get_horse, ":horse_agent", ":agent_id"),
        (try_begin),
          (eq, ":horse_agent", -1),
          (agent_set_position, ":agent_id", pos62),
        (else_try),
          (agent_set_position, ":agent_id", pos62),
          (agent_set_position, ":horse_agent", pos62),
  ...
  ("cf_player_check_spawn_agent", # server: check if the a new agent should be spawned for a player
   [(store_script_param, ":player_id", 1), # must be valid
    ...
    (try_begin),
      ...
      (try_begin),
        ...
        (try_end),
        (try_begin),
          (player_slot_eq, ":player_id", slot_player_saved_item_0, -1),
          (player_slot_eq, ":player_id", slot_player_saved_item_1, -1),
          (player_slot_eq, ":player_id", slot_player_saved_item_2, -1),
          (player_slot_eq, ":player_id", slot_player_saved_item_3, -1),
   ...
   [(store_script_param, ":player_id", 1), # must be valid
    ...
    (try_begin),
      (this_or_next|player_slot_gt, ":player_id", slot_player_saved_item_0, -1),
      (this_or_next|player_slot_gt, ":player_id", slot_player_saved_item_1, -1),
      (this_or_next|player_slot_gt, ":player_id", slot_player_saved_item_2, -1),
      (player_slot_gt, ":player_id", slot_player_saved_item_3, -1),
